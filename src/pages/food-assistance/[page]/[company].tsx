...
const CompanyPage: React.FC<CompanyPageProps> = ({
    ...
    useEffect(() => {
        let href
        for (let index = 0; index < foodAssistanceServices.length; index++) {
            const isSelectedItem = foodAssistanceServices[index].href.includes(path);
            if (isSelectedItem) {
                href = foodAssistanceServices[index].getCompanyInfo
            }
        }
