...
const CompanyPage: React.FC<CompanyPageProps> = ({
    ...
    useEffect(() => {
        let href
        for (let index = 0; index < educationServices.length; index++) {
            const isSelectedItem = educationServices[index].href.includes(path);
            if (isSelectedItem) {
                href = educationServices[index].getCompanyInfo
            }
        }
