...
const CompanyPage: React.FC<CompanyPageProps> = ({
    ...
    useEffect(() => {
        let href
        for (let index = 0; index < disabilityServices.length; index++) {
            const isSelectedItem = disabilityServices[index].href.includes(path);
            if (isSelectedItem) {
                href = disabilityServices[index].getCompanyInfo
            }
        }
