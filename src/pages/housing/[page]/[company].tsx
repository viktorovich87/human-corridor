...
const CompanyPage: React.FC<CompanyPageProps> = ({
    ...
    useEffect(() => {
        let href
        for (let index = 0; index < housingServices.length; index++) {
            const isSelectedItem = housingServices[index].href.includes(path);
            if (isSelectedItem) {
                href = housingServices[index].getCompanyInfo
            }
        }
